## Info

Name: Searidang Pa
Phone: +1 617-764-6897
Email: dangsyncpa@gmail.com
GitHub: SearidangPa

## Education

- Middlebury College, BA in Computer Science and Mathematics, 2016-2020
- Northeastern University, MS in Computer Science, 2020-2022

## Experience

- Software Engineer II
  - Spearheaded the development of
  - Design, implement,
  - Research windows cloud filter APIs
  - Lead development, main contributor, code review for team of 3

- Software Engineer I
  - Fix bugs

## Project

### Neovim plugins

I started out just tinkering some small config. One year later, I made the custom plugins below that
are tailored to make my daily workflow enjoyable.

- theme-loader: help with dynamically syncing themes between neovim, tmux and os
- Butterfly
- Blackboard
- Jira-GH: integrate github pull request with jira status
- Hop-copilot: Hop to the place within copilot inline-completion
- terminal-multiplexer: easily manage terminal daemons within neovim
- Log-reader: Read logrus files with syntax highlighting with parsing
- Go-sparrow: fast navigation to places such as function declaration, function call, variable assignment using treesitter
- Go-test-t: Test runner

### Learning Zig

I love the premise of zig. To incentivize myself to learn zig, I made the following projects:

- SlackPR
- Jira-sync: sync jira issues from git commit log
- worktree-swap:
  Swap two workree between two directories. This is very ergonomic to have two directories
  that i can alternate for different tasks.

## AI tools

- sst/Opencode with opencode.nvim within tmux
- ThePrimeagen/99: fast ai completion constrainted to a specific function or visual selection range
- agavra/tuicr cli: to help with reviewing and curating specific feedback to the code review
- SearidangPa/mini.diff: a fork of mini.diff that shows the diff inside neovim. The power comes from
  the ability to use the lsp inside the code base while viewing the diff simultaneously.
- claude code

I've also played around with warp, codex

## My current philosophy with ai tools:

it is a great tool for learning.

### Tools

- Nvim, Fish shell, Tmux, Ghostty,
